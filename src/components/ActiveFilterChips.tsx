...
export default function ActiveFilterChips({
  ...
  return (
    <div className="flex flex-wrap items-center gap-2 mt-4">
      {filters.map((filter) => (
        <span key={filter.key} className="flex items-center gap-1.5 bg-[#e9ecf6] rounded-full pl-3 pr-1.5 py-1 text-xs sm:text-sm">
          {filter.label}
          <button
            type="button"
      ...
      <button type="button" onClick={onClearAll} className="text-xs sm:text-sm text-[#183fad] underline underline-offset-2">
        Clear all
      </button>
    </div>
  );
